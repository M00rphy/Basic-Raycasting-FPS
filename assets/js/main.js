...
ge.default_eventHandler = {
    onkeydown : function(state, e) {
        ...
        e = e || window.event;
        switch (e.keyCode) {
            case 38: state.player.speed =  1.5; break; // Move forward
            case 40: state.player.speed = -1.5; break; // Move backward
            case 39:
                if (e.shiftKey) {
                    state.player.strafe =  1.5; // Strafe right
                } else {
                    state.player.dir =  1.5;  // Rotate right
            ...
            case 37:
                if (e.shiftKey) {
                    state.player.strafe = -1.5; // Strafe left
                } else {
                    state.player.dir = -1.5;  // Rotate left
                    if (state.player.rotSpeed < state.player.maxRotSpeed) {
                        state.player.rotSpeed = state.player.deltaRotSpeed(
                                                    state.player.rotSpeed);
                    }
...
ge.default_options = {
    ...
    ceilingImage          : "",    // Sky texture
    ceilingSolidColor     : "red",       // Ceiling solid color
    floorSolidColor       : undefined,  // Floor solid color
    ...
    spriteDrawOffsetX : 0.7,
    spriteDrawOffsetY : 0.7
};
...
ge.default_initial_player_state = {
    ...
    maxRotSpeed   : 4 * Math.PI / 180,   // Max rotation speed
    ...
    crossHairSize : 6, // How many pixels constitute "middle" of the screen
...
ge.default_initial_sprite_state = {
    ...
    dir : 0,  // Turning direction (-1 for left, 1 for right, 0 no turning)
    rot : 0,  // Angle of rotation
    rotSpeed : 7 * Math.PI / 180,  // Rotation speed for each step (in radians)

    speed     : 0,    // Moving direction (1 forward, -1 backwards, 0 no movement)
    strafe    : 0,    // Strafing direction of player (-1 left, 1 right, 0 no movement)
    moveSpeed : 0.20, // Move speed for each step
    ...
    spriteWidth   : 128, // Sprite size on sprite atlas
    spriteHeight  : 128, // Sprite size on sprite atlas
        ...
        while (do_h || do_v) {
            ...
            if (do_v) {
                ...
                if (wallType_v > 0) {
                    ...
                    texturex_v = y_v % 1;
                    if (!right) {
                        texturex_v = 1 - texturex_v; // (Tilesets graphical enhancement bug) replace the minus with plus to add the texture tile to the right
                    }
            ...
            if (do_h) {
                ...
                if (wallType_h > 0) {
                    ...
                    texturex_h = x_h % 1;
                    if (up) {
                        texturex_h = 1 - texturex_h; // /(Tilsets graphical enhancement bug) replace the minus with plus to add the texture tile to the right
                    }
